...
Thesis De-AI Tool
=================
Two-stage pipeline to reduce AI detection scores in academic papers.
Parses DOCX files with color-coded AI detection reports, rewrites flagged
paragraphs via DeepSeek API.

Usage:
    1. Place paper.docx and ai_report.docx in the same directory as this script
    2. Set env var: DEEPSEEK_API_KEY=sk-your-key
    3. Run: python deai_paper.py

Output: paper_deai.docx

How it works:
    Stage 1 (temp=0.95): Break AI sentence patterns -> lower detection rate
    Stage 2 (temp=0.30): Upgrade casual vocabulary -> maintain academic tone
...
RED_COLORS = {"F12828", "FF0000", "FF00000", "C00000", "E60000"}
ORANGE_COLORS = {"F39800", "FF6600", "FF8C00", "FFA500", "FF7F00"}

# ═══════════════════════════════════════════════════════
# Prompts
# ═══════════════════════════════════════════════════════
SYSTEM_PROMPT_STAGE1 = """你是论文编辑，任务是改写文本使其不被AI检测器识别为AI生成。
...
SYSTEM_PROMPT_STAGE2 = """你是词汇替换工具。下面是已经降过AI检测率的文本，句式结构很好，但部分词汇太口语化。
...
def parse_colored(filepath):
    ...
    paras = []
    for p_idx, p_elem in enumerate(root.iter(f"{{{WORDML_NS}}}p")):
        red_text, orange_text, total_text = [], [], []
        for r_elem in p_elem.iter(f"{{{WORDML_NS}}}r"):
            rpr = r_elem.find(f"{{{WORDML_NS}}}rPr")
            color = "000000"
            if rpr is not None:
                c = rpr.find(f"{{{WORDML_NS}}}color")
                if c is not None:
                    color = (c.get(f"{{{WORDML_NS}}}val") or "000000").upper().lstrip("#")
            t_elem = r_elem.find(f"{{{WORDML_NS}}}t")
            text = (t_elem.text or "") if t_elem is not None else ""
            total_text.append(text)
            if color in RED_COLORS: red_text.append(text)
            elif color in ORANGE_COLORS: orange_text.append(text)
...
def parse_clean(filepath):
    ...
    paras = []
    for p_idx, p_elem in enumerate(root.iter(f"{{{WORDML_NS}}}p")):
        texts = [t.text or "" for t in p_elem.iter(f"{{{WORDML_NS}}}t")]
...
def save_docx(original_path, output_path, paragraphs):
    ...
    all_p = list(root.iter(f"{{{WORDML_NS}}}p"))
    ...
    for p_idx, p_elem in enumerate(all_p):
        if p_idx in rmap:
            runs = list(p_elem.iter(f"{{{WORDML_NS}}}r"))
            if runs:
                for r in runs:
                    for t in r.iter(f"{{{WORDML_NS}}}t"): t.text = ""
                ft = runs[0].find(f"{{{WORDML_NS}}}t")
                if ft is not None:
                    ft.text = rmap[p_idx]
